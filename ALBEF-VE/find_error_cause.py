...
entailonly_dir = 'data/entailonly_neg'
tuple_dir = 'data/tuple_entailonly_neg'
hypothesis_dir = 'data/img_text'

cause_error_dir = 'data/entailonly_neg_error'
...
for txt_file in txt_files:
    ...
    error_cause_tuples = []
    if 'neutral' in all_text_result or 'entailment' in all_text_result:
        with open(text_path, 'r', encoding='utf-8') as f1, open(tuple_path, 'r', encoding='utf-8') as f2:
            for line1, line2 in zip(f1, f2):  # 使用zip同时迭代两个文件
                if line1.strip().split('||')[1] in ['entailment', 'neutral']:
