...
entailonly_dir = 'data/entailonly'
tuple_dir = 'data/tuple'
hypothesis_dir = 'data/img_text'

cause_error_dir = 'data/entailonly_error'
    ...
    if 'Neutral' in all_text_result or 'Contradiction' in all_text_result:
        ...
        for tuple in error_cause_tuples:
            if tuple.strip().split(',')[0] == 'entity':
                ...
                # 将实体分割成单个单词，并添加到error_words
                words = entity.split()  # 将实体按空格分割成单词
                error_words.extend(words)  # 把单词逐一添加到error_words列表中
